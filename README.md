...
администратора, после чего печатает адрес панели и пароль. Обновляться панель будет
из релизов того репозитория, из которого собран пакет: `mp update` покажет его как
«из сборки», менять настройку нужно только форку или приватному репозиторию.

Скрипту не нужен GitHub API — тег последнего релиза берётся из редиректа
`/releases/latest`, файлы качаются по прямым ссылкам, так что анонимный лимит API
(60 запросов в час на адрес) установке не мешает. Так же поступает и сама панель при
проверке обновлений, если лимит исчерпан.
...
небольшой модуль политики под хостинг. Для Ubuntu 26.04 у `ppa:ondrej/php` пока нет сборок, поэтому там
PHP берётся из `packages.sury.org` — собственного репозитория того же сопровождающего, где для 26.04 есть все
ветки; если нет и его, ставится PHP из самой Ubuntu.
...
Почта работает на Debian/Ubuntu с dovecot 2.3 и 2.4 (Debian 13, Ubuntu 26.04 — своя конфигурация
под синтаксис 2.4); для EL конфигурация ещё не написана, контент-фильтра (rspamd) нет. Переезд —
...
Справочник API живёт в самой панели: `/api/v1/docs` (OpenAPI 3.1) — после входа в панель или с API-токеном; без авторизации документация и спецификация закрыты, Stoplight Elements вшит в бинарник, сторонних скриптов на странице нет.
