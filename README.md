...
|---|---|---|
| PHP 5.6–8.5 | `mp php list\|install\|remove`, `mp php ext list\|enable\|disable` | Sury / PPA ondrej / Remi, несколько веток параллельно, свой php-fpm на каждую, `99-monopanel.ini`; расширения ветки можно выключать и включать (`phpenmod`/`phpdismod` + перезапуск php-fpm) — действует на все сайты ветки, потому что мастер php-fpm один на версию |
