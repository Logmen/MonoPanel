...
func (s *Server) mailMaps(ctx context.Context, c mailConfig) ([]agent.FileSpec, []string, error) {
	...
	var domainLines, lenientLines, users, keyTable, signingTable []string
	...
	sort.Strings(lenientLines)
	sort.Strings(boxLines)
	sort.Strings(aliasLines)
	sort.Strings(senderLines)
	sort.Strings(users)
	header := "# Generated by MonoPanel — do not edit: changes will be overwritten.\n"
	maps := []string{"domains", "mailboxes", "aliases", "senders", "lenient"}
	content := [][]string{domainLines, boxLines, aliasLines, senderLines, lenientLines}
	...
	files = append(files,
		agent.FileSpec{Path: postfixMapsDir + "/submission_header_checks", Mode: 0o644,
			Content: header + "# Mail a client sends through 587/465 must not reveal the client's address.\n/^Received:.*with ESMTPSA/ IGNORE\n/^X-Originating-IP:/ IGNORE\n"},
...
// webmailConfPath is the nginx server block that publishes the webmail on a
// port of the mail host. http.d is included at http level, so a server block
// belongs there and not among the per-domain sites.
func (s *Server) webmailConfPath() string {
	return path.Join(s.profile.Web().NginxConfDir, "monopanel", "http.d", "monopanel-webmail.conf")
}

// applyWebmailPort publishes (or unpublishes) Roundcube on its own port. The
// certificate and the name are the mail server's own, so no DNS record and no
// second certificate are needed — which is the whole point of the port.
func (s *Server) applyWebmailPort(ctx context.Context, logf func(string, ...any), c mailConfig, certPath, keyPath string) error {
	conf := s.webmailConfPath()
	web := s.profile.Web()
	drop := func() error {
		res, err := s.agent.RemovePaths(ctx, &agent.RemovePathsRequest{
			Paths: []string{conf}, Validate: [][]string{web.NginxCheckArgv}, Reload: []string{web.NginxService},
		})
		if err != nil {
			return err
		}
		if len(res.Removed) > 0 {
			logf("webmail port disabled")
		}
		return nil
	}
	if c.WebmailPort == 0 || c.Webmail == "" || certPath == "" {
		return drop()
	}
	site, err := s.db.GetSiteByDomain(ctx, c.Webmail)
	if err != nil {
	...
	body, err := s.render.Render("mail/webmail.conf.tmpl", render.Webmail{
		IP: ip, Port: c.WebmailPort, ServerName: c.Hostname, CertPath: certPath, KeyPath: keyPath,
		Docroot: l.docroot, LogDir: path.Join(l.data, "logs"), Socket: l.socket, ClientMaxBodySize: site.ClientMaxBody,
	})
	if err != nil {
		return err
	}
	if err := s.ensureSiteLogs(ctx, owner.Login, path.Join(l.data, "logs"), "webmail.access.log", "webmail.error.log"); err != nil {
		logf("warning: webmail logs: %v", err)
	}
	...
	if len(res.Written) > 0 {
		logf("webmail at https://%s:%d/", c.Hostname, c.WebmailPort)
	}
	return nil
}
...
func splitAddress(addr string) (local, domain string, err error) {
	...
	if !ok || !mailDomainRe.MatchString(domain) {
		return "", "", errors.New("address must look like user@example.com")
	}
	...
	if local != "@" && !localPartRe.MatchString(local) {
		return "", "", errors.New("a mailbox name may contain only Latin letters, digits, dots, hyphens and underscores")
	}
	if len(local) > 64 {
		return "", "", errors.New("mailbox name is longer than 64 characters")
	}
...
func (s *Server) mailDomainFor(ctx context.Context, name string) (*store.MailDomain, error) {
	...
	if errors.Is(err, store.ErrNotFound) {
		return nil, errors.New("domain " + name + " is not served by the panel")
	}
	if err != nil {
		return nil, err
	}
...
func (s *Server) mailWarnings(ctx context.Context, c mailConfig, tls string, probes map[int]probeResult) []string {
	...
	if tls == store.CertKindSelfSigned {
		out = append(out, "self-signed certificate: mail clients will complain; issue a certificate for "+c.Hostname)
	} else if tls == "none" {
		out = append(out, "no certificate: submission (587/465) is off, since passwords would travel in plain text")
	}
	if !c.Port25 {
		out = append(out, "accepting mail on port 25 is off: no mail will arrive from outside")
	}
	if missing := missingMailPorts(c, probes); len(missing) > 0 {
		out = append(out, "ports not listening: "+strings.Join(missing, ", ")+"; see journalctl -u postfix -u dovecot")
	}
	if c.DKIM && s.secrets == nil {
		out = append(out, "the panel's encryption key is unavailable: DKIM keys cannot be read")
	}
	if n, err := s.db.ListMailDomains(ctx, 0); err == nil && len(n) == 0 {
		out = append(out, "no domains added yet")
	}
	return out
}
