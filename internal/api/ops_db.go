...
// dbHost is what the panel's defaults depend on: the host's memory and the
// slow log path of the engine.
func (s *Server) dbHost(ctx context.Context, inst *store.DBInstance) (ramMB int, slowLog string, err error) {
	info, err := s.agent.SystemInfo(ctx)
	if err != nil {
		return 0, "", err
	}
	return int(info.MemTotalBytes / 1024 / 1024), osprofile.DB(s.profile, inst.Engine).SlowLog, nil
}

// writeDBConfig renders zz-monopanel.cnf for the host size with the
// administrator's settings folded in, has mysqld validate it and restarts
// the server.
func (s *Server) writeDBConfig(ctx context.Context, inst *store.DBInstance, restart bool) error {
	...
	settings, _ := dbSettings(dbDefaults(ramMB, slowLog), s.dbOverrides(ctx))
	conf, err := s.render.Render("mysql/monopanel.cnf.tmpl", render.MySQLConf{RAMMB: ramMB, Settings: settings, NativePassword: inst.NativePassword})
	if err != nil {
		return err
	}
	if s.profile.Family() == osprofile.FamilyRHEL {
		// Debian's package ships /var/log/mysql; on EL the panel makes it, and
		// the agent gives it the SELinux label mysqld expects.
		if _, err := s.agent.EnsureDirs(ctx, &agent.EnsureDirsRequest{Dirs: []agent.DirSpec{{Path: path.Dir(layout.SlowLog), Mode: 0o750, Owner: "mysql", Group: "mysql"}}}); err != nil {
			return err
		}
	}
	req := &agent.ApplyConfigSetRequest{Files: []agent.FileSpec{{Path: layout.ConfFile, Content: conf, Mode: 0o644}}, Origin: "db:config",
		// An unknown variable or an impossible value is refused here, before
		// the running server is stopped; the agent puts the old file back.
		Validate: [][]string{{mysqldBinary, "--validate-config", "--user=mysql"}}}
	if layout.IncludeFile != "" {
		req.Files = append(req.Files, agent.FileSpec{Path: layout.IncludeFile, Content: "# Generated by MonoPanel: the server reads this file by default and pulls the panel's settings in.\n!includedir " + path.Dir(layout.ConfFile) + "/\n", Mode: 0o644})
	}
	if restart {
