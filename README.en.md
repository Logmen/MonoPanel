...
| CMS | `mp cms list\|install <domain> <cms>` | installs WordPress, Joomla, OpenCart and 1C-Bitrix (trial edition) into a site: the vendor's distribution, unpacked into the docroot as the client, its own database, the CMS's own installer (wp-cli, the Joomla and OpenCart CLI installers, the Bitrix web wizard driven by the panel), the matching preset; the administrator credentials are shown once; a CMS tab on the site page |
