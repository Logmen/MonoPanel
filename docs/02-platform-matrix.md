...
- Шаблон `zz-monopanel.cnf` по объёму RAM: `innodb_buffer_pool_size` (25–50 %), `innodb_redo_log_capacity`, `max_connections`, `table_open_cache`, `tmp_table_size`; `bind-address=127.0.0.1`; `local_infile=OFF`; `performance_schema=ON`; бинлог по умолчанию выключен (`disable_log_bin`) — на single-node он только занимает диск; включается флагом с `binlog_expire_logs_seconds=259200`.
