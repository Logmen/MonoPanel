...
- [x] dovecot 2.4 (Debian 13, Ubuntu 26.04) — отдельный шаблон `mail/dovecot24.conf.tmpl` под синтаксис 2.4 (`mail_driver`/`mail_path`, именованные `passdb`/`userdb`, квота `count` с `userdb_quota_storage_size`, `sieve_script`, `ssl_server_*_file`); шаблон выбирается по установленной версии при каждом применении, так что переход ОС 2.3 → 2.4 подхватывается сам.
