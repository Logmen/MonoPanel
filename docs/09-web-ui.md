...
Скриншоты сняты с примерного сервера — Ubuntu 24.04, MonoPanel 0.8.8, сайты на
...
Node и Python под systemd. Кнопка valkey открывает два экземпляра Valkey аккаунта — для кеша
и для PHP-сессий: каждый работает от имени аккаунта с лимитом памяти и слушает только свой
unix-сокет, путь к которому показан тут же. Пароль общий для панели и SFTP. Удаление аккаунта
убирает и его сайты, базы, cron, экземпляры Valkey и сертификаты.
