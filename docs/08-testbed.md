...
`make testbed-cms NAME=<машина> [CMS="wordpress joomla opencart bitrix"]` (он же `testbed.sh cms`) ставит на машину настоящие CMS в сайты с соответствующими пресетами и проверяет, что сайт работает, а правила пресета действуют. Нужны имена в DNS (`dns up`): сайты называются `<cms>.<машина>.tb.<зона>` и живут по HTTP — HTTP-01 до частной сети не дотянется, а проверять пресеты TLS не нужен. Пользователь панели `cms`, база `cms_<cms>`, PHP — новейшая установленная ветка (на Ubuntu 26.04 и Rocky 9 это 8.5).

- **WordPress** ставится через wp-cli; проверки: главная и вход, `/wp-admin/` уводит на логин, `xmlrpc.php` закрыт (403), PHP в `wp-content/uploads/` не исполняется, ЧПУ-ссылка отдаёт 200.
- **Joomla** (последний релиз с GitHub) — через `installation/joomla.php install`; проверки: главная, `/administrator/`, `configuration.php` и `cache/` закрыты, `/api/` отвечает 401, PHP в `images/` не исполняется, SEF-маршрут работает.
- **OpenCart** (последний релиз с GitHub) — через `install/cli_install.php`; проверки: витрина, `/admin/`, `system/` и `storage/` закрыты, `.twig` не отдаётся, SEO-URL уходит в `_route_`.
- **1С-Битрикс** — пробная редакция «Старт» (`start_encode.tar.gz` с 1c-bitrix.ru, 313 МБ). CLI-установщика у Битрикса нет, поэтому `bitrix-wizard.py` проходит веб-мастер по HTTP как браузер: соглашение, пробная регистрация, проверка требований (её делает сам Битрикс — на неверных настройках PHP мастер дальше не идёт), существующая база, установка модулей и обновлений (AJAX-шаги с подсказками `Post({...})` в ответах), администратор, встроенное демо «Мебельная компания». Проверки: `X-Powered-CMS: Bitrix Site Manager` на главной, форма входа `/bitrix/admin/`, закрытые `php_interface/dbconn.php`, `modules/`, `.settings.php`, `cache/`, PHP в `upload/` не исполняется, несуществующий адрес обрабатывает `urlrewrite.php` (404 от Битрикса), `mp site php` показывает `short_open_tag On`, `max_input_vars 20000`, `memory_limit 512M`. Мастер укладывается в минуту.

Доступы к админкам печатаются строками `CREDS` один раз. Прогон 2026-09-12: все четыре CMS на всех одиннадцати ОС, 70 проверок на машину без ошибок. Что пришлось учесть в самом сценарии: на EL Percona идёт с `validate_password` (пароль базы должен содержать все классы символов, на Debian плагина нет — панель отдаёт ошибку MySQL как есть); файлы, скопированные `cp -a` из `/root`, на EL сохраняют метку SELinux `admin_home_t` и nginx отдаёт 403 — нужен `restorecon -R` (архивы, распакованные `tar` прямо в docroot, метятся верно); `mp db create` для существующей базы не ошибается, а переустанавливает пароль.
