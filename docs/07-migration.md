...
> Проектный документ. Первый этап — прямой перенос между двумя MonoPanel — реализован (`mp migrate`); адаптеры для чужих панелей и пакет-файл описаны здесь как проект. Что работает — в [README](../README.md) и отметках [x] в [05-roadmap.md](05-roadmap.md).
...
Чего в первом этапе нет: досинхронизации (`resync`), завершения переезда (`finish`), областей `site:` и `server:`, пакета-файла и адаптеров для чужих панелей.
