<script lang="ts">
  ...
  const info: Record<string, { title: string; text: string }> = {
    ...
    sphinx: { title: 'Sphinx', text: 'полнотекстовый поиск для 1С-Битрикс: на Debian/Ubuntu — Sphinx 2.2 из дистрибутива, на EL — Sphinx 3 с sphinxsearch.com; индекс bitrix, SphinxQL на 127.0.0.1:9306. В Битриксе: Настройки → Поиск → Sphinx, строка подключения 127.0.0.1:9306' }
  };
  ...
  const askInstall = (name: string): Ask => ({
    ...
    note: name === 'composer' ? 'composer.phar скачается с getcomposer.org, контрольная сумма сверится с опубликованной. Запускаться будет на новейшей установленной ветке PHP; повторная установка обновляет его.' :
      ...
      name === 'sphinx' ? 'Установится сервер поиска с готовым индексом bitrix (все атрибуты по документации Битрикса) и SphinxQL на 127.0.0.1:9306. На EL скачается сборка Sphinx 3 с sphinxsearch.com с проверкой контрольной суммы.' :
