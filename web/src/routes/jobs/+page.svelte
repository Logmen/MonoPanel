...
<div class="card overflow-x-auto p-0 rise">
  ...
  <table class="tbl"><thead><tr><th>ID</th><th>Тип</th><th>Статус</th><th class="w-28">Прогресс</th><th>Кто</th><th>Создана</th><th>Сообщение</th></tr></thead>
    <tbody>
      {#each shown as j, i}<tr class="cursor-pointer rise {open === j.id ? 'bg-accent-soft/40' : ''}" style="--i:{Math.min(i, 12)}" onclick={() => (open = j.id)}><td data-label="ID" class="font-mono text-muted">{j.id}</td><td data-label="Тип" class="font-mono">{j.type}</td><td data-label="Статус"><span class="tag {j.status === 'done' ? 'tag-ok' : j.status === 'failed' ? 'tag-err' : j.status === 'running' ? 'tag-accent' : 'tag-warn'}">{#if j.status === 'running'}<span class="dot dot-live"></span>{/if}{j.status}</span></td><td data-label="Прогресс"><div class="progress {j.status === 'running' ? 'running' : ''}"><i style="width:{j.progress}%"></i></div></td><td data-label="Кто" class="text-muted">{j.requested_by}</td><td data-label="Создана" class="text-xs text-muted whitespace-nowrap">{when(j.created_at)}</td><td data-label="Сообщение" class="text-xs max-w-md truncate {j.error ? 'text-danger' : 'text-muted'}" title={j.error || j.message}>{j.error || j.message}</td></tr>{/each}
