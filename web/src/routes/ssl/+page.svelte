  ...
  <table class="tbl"><thead><tr><th>Имя</th><th>SAN</th><th>Статус</th><th>Издатель</th><th>Истекает</th><th>Авто</th><th></th></tr></thead>
    <tbody>
      {#each certs as c, i}
        ...
        <tr class="rise" style="--i:{i}"><td class="font-mono font-medium">{c.name}</td><td class="text-xs text-muted max-w-xs truncate" title={c.names.join(', ')}>{c.names.join(', ')}</td><td><span class="tag {c.status === 'valid' ? 'tag-ok' : c.status === 'error' ? 'tag-err' : 'tag-warn'}">{c.status}</span>{#if c.last_error}<div class="text-xs text-danger max-w-xs truncate" title={c.last_error}>{c.last_error}</div>{/if}</td><td class="text-muted">{c.issuer || '—'}</td><td class="text-xs"><span class="tag {d < 14 ? 'tag-err' : d < 30 ? 'tag-warn' : 'tag-muted'}">{daysLeft(c.not_after)}</span></td><td>{c.auto_renew ? 'да' : 'нет'}</td><td><div class="row-actions"><button class="btn btn-sm" onclick={() => renew(c.id)}><Icon name="refresh" size={13} /> продлить</button><button class="btn btn-danger btn-sm" onclick={() => (del = c)}><Icon name="trash" size={13} /></button></div></td></tr>
      {/each}
