<script lang="ts">
  ...
  const askDisable = (): Ask => ({
    title: 'Выключить firewall?',
    note: 'Таблица nftables будет снята целиком: снаружи станут доступны все порты, которые слушают сервисы сервера, а не только 80, 443, SSH и панель. Правила и баны сохранятся и вернутся при включении.',
    danger: true, action: 'Выключить',
    run: () => act('disable')
  });
  const askRule = (r: any): Ask => ({
    title: 'Удалить правило?',
    note: `${r.kind === 'allow' ? 'Разрешение' : 'Запрет'} ${r.proto} ${r.port || 'любой порт'}${r.source ? ' для ' + r.source : ''} пропадёт сразу после применения таблицы.`,
    danger: true, action: 'Удалить',
    run: () => rm(r.id)
  });
  const askUnban = (ip: string): Ask => ({
    title: `Разбанить ${ip}?`,
    note: 'Адрес снимут из бана fail2ban, и он снова сможет подключаться. Если перебор продолжится, jail забанит его заново.',
    action: 'Разбанить',
    run: () => doBan('unban', ip)
  });
...
<PageHead title="Firewall" sub={fw ? `nftables inet monopanel · policy drop · всегда открыты SSH ${fw.ssh_ports.join(',')}, 80, 443, панель ${fw.panel_port}` : ''}>
  {#if fw}
    ...
    {#if fw.enabled}<button class="btn" onclick={() => act('apply')}><Icon name="refresh" size={14} /> применить</button><button class="btn btn-danger" onclick={() => (ask = askDisable())}>выключить</button>{:else}<button class="btn btn-primary" onclick={() => act('enable')}>включить</button>{/if}
  {/if}
</PageHead>
